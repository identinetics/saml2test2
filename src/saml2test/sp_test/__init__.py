...
from aatest import tool, FatalError, CheckError
...
class Conversation(tool.Conversation):
    ...
    def send(self):
        srvs = getattr(self.entity.metadata, REQ2SRV[self.oper.request])(
    ...
    def _send(self, srv):
        ...
        if use_artifact:
            saml_art = _entity.use_artifact(str_req, self.args["entity_id"])
        ...
        if self.args["request_binding"] is BINDING_SOAP:
            res = _entity.send_using_soap(str_req, loc)
            ...
            if self.args["request_binding"] is BINDING_HTTP_REDIRECT:
                ...
                #
                res = _entity.send(htargs["headers"][0][1], "GET")
                ...
                res = _entity.send(loc, "POST", **htargs)
            elif self.args["request_binding"] == BINDING_URI:
                self.response_args["binding"] = BINDING_URI
                htargs = _entity.use_http_uri(str_req, "SAMLRequest", loc)
                res = _entity.send(htargs["url"], "GET")
            else:
                res = None
    ...
    def setup_request(self):
        ...
        self.response_func = getattr(_entity, "parse_%s_response" % query)
        qargs = self.args.copy()
        self.relay_state = rndstr()

        if "message" not in _oper.args:
            self.qfunc = getattr(_entity, "create_%s" % query)
            # remove args the create function can't handle
            fargs = inspect.signature(self.qfunc).args
            if _oper._class:
    ...
    def my_endpoints(self):
        return [e for e, b in self.entity.config.getattr("endpoints", "sp")[
        ...
        try:
            ...
            if isinstance(response, dict):
                ...
                if "SAMLResponse" in response:
                    ...
                    response = self.entity.artifact2message(
